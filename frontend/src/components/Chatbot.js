...
const Chatbot = () => {
  ...
  const toggleListening = () => {
    if (isListening) {
      ...
      try {
        recognition.start();
        setIsListening(true);
        console.log('Microphone started, please speak...');
      } catch (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (language !== 'en') {
        translatedInput = await translateText(input, 'en', language);
      }
      ...
      console.log('RAG full response:', response.data);
      console.log('RAG response answer:', response.data?.answer);
